...
public class ValidationExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handleMessageNotReadable(HttpMessageNotReadableException ex) {
        ...
        String rootMsg = root != null && root.getMessage() != null ? root.getMessage() : (msg != null ? msg : "");

        if (rootMsg.contains("ReservationStatus") || rootMsg.contains("not one of the values accepted for Enum class")) {
            String detail = "Invalid ReservationStatus value. Allowed values: [CONFIRMED, ARRIVED, COMPLETED, NO_SHOW, CANCELLED]";
            ProblemDetail pd = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST, detail);
            pd.setType(URI.create("https://example.invalid/problems/invalid-reservationstatus"));
            pd.setTitle("Invalid ReservationStatus");
            return ResponseEntity.badRequest().body(pd);
        }
